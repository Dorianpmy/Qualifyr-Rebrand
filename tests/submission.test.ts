...
const validDiagnostic = {
  ...
  website: '',
  seniority: '1-3-ans',
  bookingMethods: ['telephone'],
  priority: 'simplifier-reservations',
  blocker: 'Trop d’allers-retours avant de fixer un rendez-vous.',
  message: '',
  consent: true,
  fax: '',
  elapsedMs: 9000,
...
describe('POST /api/diagnostic — succès', () => {
  ...
  it('inclut les informations attendues dans l’e-mail reçu par Qualifyr', async () => {
    ...
    for (const expected of [
      'Diagnostic',
      'Reçue le',
      validDiagnostic.fullName,
      validDiagnostic.email,
      validDiagnostic.company,
      validDiagnostic.area,
      'Objectif prioritaire',
      'Réservations reçues par',
      validDiagnostic.blocker,
